...
class Mediator(EventMediator, QueryMediator, CommandMediator):
    ...
    async def publish(self, events: Iterable[Event]) -> Iterable[Any]:
        ...
        for event in events:
            ...
            for handler in handlers:
                handler_instance: EventHandler = self.container.resolve(handler) # type: ignore
    ...
    async def handle_command(self, command: Command) -> Iterable[Any]:
        ...
        result = []
        for handler in handlers:
            handler_instance: CommandHandler = self.container.resolve(handler) # type: ignore
            if hasattr(handler_instance, "handle"):
    ...
    async def handle_query(self, query: Query) -> Any:
        ...
        if handler_class:
            handler: QueryHandler = self.container.resolve(handler_class) # type: ignore
